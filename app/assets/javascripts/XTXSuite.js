...
function $AttachIE11OnUSBKeychangeEvent(strObjName) {
	...
	var messageNode = document.createTextNode("$OnUsbKeyChange()");
	handler.appendChild(messageNode);
}
...
// 2015.05.12 添加GetKeyPic控件的支持
function $LoadGetKeyPic()
{
	...
	o.GetPic = function(strContainerName) {
		if (arguments.length == 0) {
			return OGetPic.GetPic();
		}
		
		return OGetPic.GetPic(strContainerName);
	};
	
	o.Hash = function(strInData) {
		return OGetPic.Hash(strInData);
	};
	
	o.ConvertPicFormat = function(strInData, type) {
		return OGetPic.ConvertPicFormat(strInData, type);
	};
	
	o.ConvertGif2Jpg = function(strInData) {
		return OGetPic.ConvertGif2Jpg(strInData);
	};
	
	o.ConvertPicSize = function(strPic, width, height) {
		return OGetPic.ConvertPicSize(strPic, width, height);
	};
	
	return o;
}

function $checkGetKeyPic()
{
	if ($_$GetKeyPic == null) {
		return false;
	} else {
...
function GetPic(strContainerName)
{
	if (!$checkGetKeyPic()) {
		return "";
	} 
	...
	if (arguments.length == 0) {
		return $_$GetKeyPic.GetPic();
	}
	var pos = strContainerName.indexOf("/");
	if (pos <= 0 ) {
		return $_$GetKeyPic.GetPic(strContainerName);
	} else {
		var strRealContainer = strContainerName.substring(0, pos);
		return $_$GetKeyPic.GetPic(strRealContainer);
	}	
}

function ConvertPicFormat(strPic, type)
{
	if (!$checkGetKeyPic()) {
		return "";
	} 

	return $_$GetKeyPic.ConvertPicFormat(strPic, type);
}

function ConvertGif2Jpg(strGif)
{
	if (!$checkGetKeyPic()) {
		return "";
	} 

	return $_$GetKeyPic.ConvertGif2Jpg(strGif);
}

function ConvertPicSize(strPic, width, height)
{
	if (!$checkGetKeyPic()) {
		return "";
	} 

	return $_$GetKeyPic.ConvertPicSize(strPic, width, height);
}
