    ...
    if self.sp_s_16.blank? and self.sp_i_state == ::PadSpBsb::Step::DEPLOYED
			...
			loop do
				...
				if self.sys_province_id == -1
					...
					@id_parts.push(SysProvince.level1.find(self.sys_province_id).code)
				end
				@id_parts.push(JgBsb.find(self.jg_bsb_id).code)
				count = 3000 + PadSpBsb.where(:sys_province_id => self.sys_province_id, :jg_bsb_id => self.jg_bsb_id).count + i
