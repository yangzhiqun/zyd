...
class TaskJgBsb < ActiveRecord::Base
  ...
  def check_quota_amount
    ...
    if self.sys_province_id != -1
      prov_quota = TaskProvince.where(identifier: self.identifier, a_category_id: self.a_category_id).sum('quota')
      current_prov_amount = TaskJgBsb.where(identifier: self.identifier, a_category_id: self.a_category_id, sys_province_id: self.sys_province_id).sum('quota')
