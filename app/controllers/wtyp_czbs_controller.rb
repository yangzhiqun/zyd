      ...
      when 0
        ...
        if params[:state].to_i == 0
          ...
          if current_user.hcz_admin != 1
            ...
            @sp_bsbs = @sp_bsbs.where("id IN (SELECT DISTINCT sp_bsb_id FROM wtyp_czb_parts WHERE (sp_bsb_id IS NULL OR current_state = 0 OR current_state IS NULL) AND (((wtyp_czb_type = 2 OR wtyp_czb_type = 3) AND sp_s_4 = ?) OR (wtyp_czb_type = 1 AND sp_s_220 = ?))) AND (((sp_s_4 = ? OR sp_s_220 = ?) AND (sp_s_68 = '流通' OR sp_s_68 = '餐饮')) OR (sp_s_220 = ? AND sp_s_68 = '生产'))", current_user.user_s_city, current_user.user_s_city, current_user.user_s_city, current_user.user_s_city, current_user.user_s_city)
          else
