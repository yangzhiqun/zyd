class BaosongAsNewestsController < ApplicationController
  # GET /baosong_as
  # GET /baosong_as.json
  def index
  ...
  def create
    ...
    respond_to do |format|
      if @baosong_a.save
        format.html { redirect_to "/baosong_as_newests/#{@baosong_a.id}", notice: 'Baosong a was successfully created.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to baosong_as_newests_url }
      format.json { head :no_content }
    end
  end
